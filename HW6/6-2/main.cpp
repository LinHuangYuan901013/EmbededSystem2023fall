...
void timer_count_callback100()
{
    for (int i=0;i<128;i++)   printf("%d ", sample_buffer[i]);
    printf("\n");     
    
}
void timer_count_callback200()
{
    for (int i=128;i<256;i++)   printf("%d ", sample_buffer[i]);    
    printf("\n"); 
...
{
    event_queue.call(timer_count_callback200);
...
void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    event_queue.call(timer_count_callback100);
}
