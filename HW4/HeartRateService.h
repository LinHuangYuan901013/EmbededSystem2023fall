...
public:
     ...
     */
    HeartRateService(BLE &_ble, uint16_t hrmCounter, BodySensorLocation location,int16_t pdata[]) :
        ble(_ble),
        valueBytes(hrmCounter,pdata),
        hrmRate(
     ...
     * BLE stack.
     */
    void updateHeartRate(uint16_t hrmCounter,int16_t pdata[]) {
        valueBytes.updateHeartRate(hrmCounter,pdata);
...
protected:
    ...
    struct HeartRateValueBytes {
        ...
        HeartRateValueBytes(uint16_t hrmCounter,int16_t pdata[]) : valueBytes()
        {
            updateHeartRate(hrmCounter,pdata);
        }
        ...
        void updateHeartRate(uint16_t hrmCounter,int16_t pdata[])
        {
            ...
            // }
            // valueBytes[0]=pdata[0];
            // valueBytes[1]=pdata[0]>> 8;
            // valueBytes[2]=pdata[1];
            // valueBytes[3]=pdata[1]>>8;
            // valueBytes[4]=pdata[2];
            // valueBytes[5]=pdata[2]>>8;
// printf("\nACCELERO_X = %d\n", pdata[0]);
// printf("\nACCELERO_Y = %d\n", pdata[1]);
// printf("\nACCELERO_Z = %d\n", pdata[2]);
            valueBytes[0]=pdata[0];
